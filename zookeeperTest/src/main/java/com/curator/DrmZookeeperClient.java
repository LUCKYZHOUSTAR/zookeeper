...
public class DrmZookeeperClient {
    private static Logger logger = LoggerFactory.getLogger(DrmZookeeperClient.class);

	private String zkAddress="100.66.162.37:2181";
	...
	public String watherPath(String path,CuratorWatcher watcher) throws Exception{
...
//                    ReflectionUtils.writeField(drmNode.getParmname(), drmNode.getObj(), value);
			...
			//监控根目录
			watherPath(path, new DrmIPWatcher(this, drmNode.getObj(), drmNode.getParmname(), true));
			//ippath
			...
			//挂载临时节点
			   //挂载临时节点监听
			watherPath(ippath,new DrmIPWatcher(this, drmNode.getObj(), drmNode.getParmname(),false));
            if(addset){
            	confset.add("");
            }
		} catch (Exception e) {
